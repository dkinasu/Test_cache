...
//			printf("line[%8lld] RID: %s [dataNum: %3lld] P_num[%5lld]--> ACCESS Node[%3d]  [Corrupted]\n", total_line, T_line->RequestID, datapblk_num, pblk_num, CurNode);
			...
			if(Check_F_Cache(cp, pblk_num))//F_cache hit
			{
				F_cache_hit++;
				T_line->Finish_Time = sys_last_busy_time;
			}
			...
			{
				//recover
				long long t = Degraded_read(pblk_num, cp);
...
//			printf("%5lld\n", Cluster[Cur_node]->Last_Busy_Time);
		...
		{
			if(Check_F_Cache(cp, tmp[i]))//F_cache hit
			{
				F_cache_hit++;
...
struct Result Process(char **files, int trace_start, int trace_end, struct traceline *T_line, int ec, int policy, int iteratimes, int degraded, int cache_policy)
{
	...
	switch(cache_policy)
	{
		case 0:
			...
			break;
		case 1:
			res.cache_name = (char *)"LRU";
			break;
	    case 2:
	    	res.cache_name = (char *)"ARC";
	    	break;
	    case 3:
	    	res.cache_name = (char *)"2-in LRU";
	    	break;
	    case 4:
	    	res.cache_name = (char *)"2-in ARC";
	    	break;
	    case 5:
